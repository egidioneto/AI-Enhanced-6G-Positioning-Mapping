...
def process_readings(readings, squares, filename, max_filename):
    """
    Process readings to create a matrix where each row represents a reading and each column represents a square,
    with normalized values based on the greatest distance found.
    ...
    """
    # Initialize the processed readings array
    processed_readings = np.zeros((len(readings), len(squares)))

    # Iterate over each sublist of readings
    for i, sublist in enumerate(readings):
        # Iterate over each reading in the sublist
        for reading in sublist:
            x, y = reading  # Unpack the tuple directly

            # Iterate over each square and accumulate the distance
            for j, (x1, x2, y1, y2) in enumerate(squares):
                if x1 <= x < x2 and y1 <= y < y2:
                    distance = np.sqrt(x**2 + y**2)  # Calculate the distance from the origin
                    processed_readings[i][j] += distance  # Sum the distances within the square
                    break

    # Find the maximum sum of distances in any square
    max_sum_distance = processed_readings.max()

    # Normalize the processed readings by the maximum sum of distances
    processed_readings /= max_sum_distance
                
    # Save the processed readings to a file
    np.savetxt(f"{filename}.csv", processed_readings, delimiter=',')
    
    # Save the max sum distance to a text file
    with open("max_sum_distance.txt", "a") as f:
        f.write(f"Max sum distance for {max_filename}: {max_sum_distance}\n")
...
for n in [16, 64]:
    ...
    for file in files:
        ...
        X = process_readings(readings, squares, f"{results_folder}/input_{n}_{file}", f"n_{n}_{file}")  # Input
        ...
        save_model(model, f"{results_folder}/run_3-{filename}")
