...
int readings_taken = 0; //Add a reading limit if you want to
...
int main(int argc, const char * argv[]) {
    ...
    /* ----------------------------- Taking readings ---------------------------- */
    // Delay for 10 seconds the processes of taking readings
    printf("Delaying for 10 seconds before taking readings...\n");
    for (int i = 10; i > 0; i--) {
    ...
    strftime(filename, sizeof(filename), "scan_2d_%Y%m%d_%H%M%S.txt", timeinfo);
    FILE *output_file;
    output_file = fopen(filename, "a");

    printf("Saving readings to file: %s\n", filename);
    while (1) {
        ...
        if (SL_IS_OK(op_result)) {
            ...
            for (int pos = 0; pos < (int)count ; ++pos) {
                fprintf(output_file, "%s theta: %03.2f Dist: %08.2f Q: %d \n", 
